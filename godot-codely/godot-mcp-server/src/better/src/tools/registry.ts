...
// [sovereign] coding-solo 独有工具 + image-gen 原生工具（单一注册点挂载）
import {
  CS_UNIQUE_TOOLS,
  dispatchCsUnique,
  type SovereignCtx,
} from '../../../cs-unique/tools.js'
import { createImageGenTools } from '../../../image-gen/image-gen.js'
...
// ---------------------------------------------------------------------------
// [sovereign] 额外工具挂载点：coding-solo 独有工具 + image-gen 原生工具。
// 由 registerTools(server, config, ctx) 在 ctx 提供时写入，单一 McpServer / 单一注册 / 无前缀。
// ---------------------------------------------------------------------------
let EXTRA_TOOLS: any[] = []
let EXTRA_DISPATCH: ((name: string, args: Record<string, unknown>) => Promise<any>) | null = null

/**
 * Register all tools with the MCP server
 */
export function registerTools(server: Server, config: GodotConfig, ctx?: SovereignCtx): void {
  // [sovereign] 挂载 coding-solo 独有工具 + image-gen 原生工具（ctx 缺省时仅 better 17 复合工具）
  if (ctx) {
    const imgTools = createImageGenTools(() => ({
      config: { volcArkApiKey: ctx.volcArkApiKey },
      projectPath: ctx.godotProjectPath,
    }))
    EXTRA_TOOLS = [
      ...CS_UNIQUE_TOOLS,
      ...imgTools.map((t) => ({ name: t.name, description: t.description, inputSchema: t.inputSchema })),
    ]
    EXTRA_DISPATCH = async (name: string, args: Record<string, unknown>) => {
      if (CS_UNIQUE_TOOLS.some((t) => t.name === name)) {
        return dispatchCsUnique(name, args, ctx)
      }
      const img = imgTools.find((t) => t.name === name)
      if (img) {
        const text = await img.handler(args)
        return { content: [{ type: 'text', text }] }
      }
      throw new GodotMCPError(`Unknown extra tool: ${name}`, 'INVALID_ACTION', 'Internal dispatch error.')
    }
  } else {
    EXTRA_TOOLS = []
    EXTRA_DISPATCH = null
  ...
  const allTools = ctx ? [...TOOLS, ...EXTRA_TOOLS] : TOOLS
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    // [sovereign] 额外工具（image-gen / cs-unique）优先分发，再回退 better 复合工具
    if (ctx && EXTRA_DISPATCH && EXTRA_TOOLS.some((t) => t.name === name)) {
      try {
        return EXTRA_DISPATCH(name, args)
      } catch (error) {
